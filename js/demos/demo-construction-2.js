...
(function ($) {
	...
	$('.thumb-gallery-detail').each(function () {
		var $detail = $(this),
			$thumbs = $detail.siblings('.thumb-gallery-thumbs');
		...
		$detail
			...
			.on('changed.owl.carousel', function (e) {
				if (!flag) {
					flag = true;
					$thumbs.trigger('to.owl.carousel', [e.item.index - 1, duration, true]);
					flag = false;
				}
			});
		...
		$thumbs
			...
			.on('click', '.owl-item', function () {
				$detail.trigger('to.owl.carousel', [$(this).index(), duration, true]);
			})
			.on('changed.owl.carousel', function (e) {
				if (!flag) {
					flag = true;
					$detail.trigger('to.owl.carousel', [e.item.index, duration, true]);
					flag = false;
				}
			});
	});
